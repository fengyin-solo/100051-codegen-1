"""联锁设备接口：维护联锁设备，覆盖确认检修、降级登记、停用设备等动作。"""
...
def list_entries(
    ...
    """按设备编号与状态过滤联锁设备列表；没有数据时返回空页，不报错。"""
    if size > 200:
...
def run_action(entry_id: int, payload: EntryPayload) -> ActionResult:
    """对单条联锁设备执行确认检修、降级登记、停用设备；不允许的动作会被拦下并说明原因。"""
