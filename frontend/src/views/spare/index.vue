<template>
  <section class="page" data-module="spare">
    <header class="page-head">
      <div>
        <h2>器材领用管理</h2>
        <p class="page-desc">维护器材领用单，围绕领用单号、器材名称、器材规格、领用数量做登记、筛选与状态流转。</p>
      </div>
    ...
    <table class="data-table">
      ...
      <tbody>
        ...
        <tr v-if="!rows.length">
          <td :colspan="columns.length + 1" class="empty-state">暂无器材领用数据，可先登记器材领用单</td>
        </tr>
      </tbody>
    </table>
...
function openCreate() {
  errorMessage.value = '器材领用单登记入口尚未接入审批流'
}
...
async function reload() {
  ...
  try {
    ...
    errorMessage.value = error instanceof Error ? error.message : '器材领用列表读取失败'
  }
}
